...
def value_iteration(mdp, discount=0.99, theta=0.01):
    ...
    # Iterate until convergence
    while True:
        # We need a copy in order to calculate some deltas
        V_new = V.copy()
        # Iterate over all states
        for s in range(mdp.num_states):
            Q_greedy_s = np.zeros(mdp.num_actions)
            # Iterate over all actions
            for a in range(mdp.num_actions):
                # Iterate over all possible outcomes of action a in state s
                for prob, next_state, reward, done in mdp.P[s][a]:
                    Q_greedy_s[a] += prob * (reward + discount * V[next_state])
            # Update
            V_new[s] = np.max(Q_greedy_s)
        # Compute deltas
